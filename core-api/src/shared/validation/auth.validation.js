...
const registerSchema = Joi.object({
  ...
  contrasena: Joi.string().min(8).max(128).required().messages({
    'string.min': 'La contraseña debe tener al menos 8 caracteres',
    'string.max': 'La contraseña no puede superar 128 caracteres',
    'any.required': 'La contraseña es requerida',
  }),
  nombres: Joi.string().trim().min(2).max(50).required().messages({
    'string.min': 'El nombre debe tener al menos 2 caracteres',
    'any.required': 'El nombre es requerido',
  }),
  apellidos: Joi.string().trim().min(2).max(50).required().messages({

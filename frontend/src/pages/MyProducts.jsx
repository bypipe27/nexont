...
const STYLES = `
  ...
  .nxmp-card-foot { border-top:1px solid rgba(26,23,20,0.06); padding-top:0.65rem; margin-top:auto; display:flex; align-items:center; justify-content:space-between; }
  .nxmp-avail { font-size:0.65rem; font-weight:600; }
  .nxmp-avail.ok { color:#16A34A; }
  .nxmp-avail.out { color:#DC2626; }
  .nxmp-edit { font-size:0.65rem; color:var(--ink-ghost); cursor:pointer; transition:color 0.15s; letter-spacing:0.08em; text-transform:uppercase; }
  .nxmp-edit:hover { color:var(--ink); }
...
`;
if (!document.getElementById('nxmp-styles')) { const el=document.createElement('style'); el.id='nxmp-styles'; el.textContent=STYLES; document.head.appendChild(el); }
...
function PublishModal({ isOpen, onClose, onPublished }) {
  ...
  const handleClose = () => { setForm({titulo:'',descripcion:'',precio:'',stock:'',condicion:'NUEVO',promedioCalificacion:'0'}); setImageFile(null); setImagePreview(null); setError(''); setSuccess(''); onClose(); };
  ...
  useEffect(() => { const h=e=>{if(e.key==='Escape')handleClose();}; if(isOpen)window.addEventListener('keydown',h); return ()=>window.removeEventListener('keydown',h); }, [isOpen]);
        ...
        <div className="nxpm-body">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="nxpm-f">
              ...
              <div className="nxpm-img-zone" onClick={() => fileInputRef.current && fileInputRef.current.click()}>
...
function MyProducts() {
  ...
  const fetchMyProducts = async (params={}) => {
    if (fetching) return;
    try { setFetching(true); setLoading(true); setError(''); const { data }=await api.get('/products/my',{params}); setProducts(data.products||[]); }
    catch (err) { if(err.response?.status===401){setError('Sesión expirada.');navigate('/login');}else setError(err.response?.data?.error||'Error al cargar tus productos'); }
    finally { setLoading(false); setFetching(false); }
  };

  useEffect(() => { if(!token||!user?.esVendedorVerificado){navigate('/');return;} fetchMyProducts(); }, [token]);
  useEffect(() => { const h=e=>{if(!e.target.closest('.nxmp-user-pill'))setDdOpen(false);}; document.addEventListener('click',h); return ()=>document.removeEventListener('click',h); }, []);
  ...
  return (
    <div className="nxmp-root">
      <header className="nxmp-bar">
        <div className="nxmp-brand" onClick={()=>navigate('/')}><img src="/resources/icone.png" alt="Nexont" /><span className="nxmp-brand-name">Nexont</span></div>
        <div className="nxmp-sep" /><span className="nxmp-bar-title">Mis Productos</span>
        <div className="nxmp-gap" />
        <div className="nxmp-bar-right">
          <Link to="/" style={{height:38,padding:'0 1rem',background:'transparent',border:'1px solid var(--border)',color:'var(--ink-soft)',fontSize:'0.72rem',letterSpacing:'0.1em',textTransform:'uppercase',cursor:'pointer',transition:'all 0.18s',textDecoration:'none',display:'inline-flex',alignItems:'center',fontFamily:"'DM Sans',sans-serif"}} onMouseEnter={e=>{e.currentTarget.style.background='var(--ink)';e.currentTarget.style.color='var(--cream)';}} onMouseLeave={e=>{e.currentTarget.style.background='transparent';e.currentTarget.style.color='var(--ink-soft)';}}>← Tienda</Link>
          ...
          <div className="nxmp-user-pill" onClick={()=>setDdOpen(o=>!o)}>
            ...
            {ddOpen && (
              <div className="nxmp-dd">
                <div className="nxmp-dd-sec"><div className="nxmp-dd-lbl">Mi cuenta</div><div className="nxmp-dd-item">👤 Mi perfil</div><div className="nxmp-dd-item" onClick={()=>navigate('/orders')}>📦 Mis órdenes</div></div>
                <div className="nxmp-dd-sec"><div className="nxmp-dd-lbl">Vendedor</div><div className="nxmp-dd-item" onClick={()=>setShowModal(true)}>➕ Nuevo producto</div><div className="nxmp-dd-item" onClick={()=>navigate('/')}>🏪 Ver catálogo</div></div>
                <div className="nxmp-dd-sec"><div className="nxmp-dd-item danger" onClick={()=>{localStorage.removeItem('token');localStorage.removeItem('user');window.location.reload();}}>🚪 Cerrar sesión</div></div>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="nxmp-page">
        ...
        <div className="nxmp-layout">
          <aside className="nxmp-sidebar">
            ...
            <div className="nxmp-sb-sec"><span className="nxmp-sb-sec-title">Buscar</span><input className="nxmp-sb-search" placeholder="Nombre del producto…" value={searchTerm} onChange={e=>setSearch(e.target.value)} onKeyDown={e=>e.key==='Enter'&&doSearch()} /></div>
            <div className="nxmp-sb-sec"><span className="nxmp-sb-sec-title">Estado</span>{['','nuevo','usado','reacondicionado'].map(c=><label key={c} className="nxmp-radio"><input type="radio" name="mpcond" value={c} checked={fCond===c} onChange={()=>setFCond(c)} /><span>{c===''?'Todos':c.charAt(0).toUpperCase()+c.slice(1)}</span></label>)}</div>
            <div className="nxmp-sb-sec"><span className="nxmp-sb-sec-title">Precio máximo</span><input type="range" min="0" max="1000" step="10" value={fMaxPrice} onChange={e=>setFMaxPrice(Number(e.target.value))} className="nxmp-range" /><div className="nxmp-range-row"><span>$0</span><span className="nxmp-range-val">${fMaxPrice}</span><span>$1000+</span></div></div>
            <div className="nxmp-sb-btns"><button className="nxmp-sb-apply" onClick={doSearch}>Aplicar filtros</button><button className="nxmp-sb-clear" onClick={doClear}>Limpiar</button><button className="nxmp-sb-new" onClick={()=>setShowModal(true)}>+ Nuevo producto</button></div>
          </aside>
          <div className="nxmp-main">
            ...
            <div className="nxmp-toolbar">
              ...
              <select className="nxmp-sortsel" value={sortBy} onChange={e=>setSortBy(e.target.value)}>
                <option value="newest">Más reciente</option><option value="price-low">Menor precio</option><option value="price-high">Mayor precio</option><option value="rating">Mejor calificación</option>
              </select>
            </div>
            {loading ? <div style={{textAlign:'center',padding:'3rem',color:'var(--ink-ghost)',fontSize:'0.78rem',letterSpacing:'0.12em',textTransform:'uppercase'}}>Cargando…</div>
              : sorted.length===0 ? (
                  ...
                  {sorted.map(p => (
                    <div key={p.id} className="nxmp-card">
                      <div className="nxmp-card-img">
                        {p.imagenes?.[0]?.url ? <img src={p.imagenes[0].url} alt={p.titulo} /> : <div className="nxmp-card-noimg">Sin imagen</div>}
                        <span className="nxmp-card-stock-badge">Stock: {p.stock}</span>
                      </div>
                      <div className="nxmp-card-body">
                        <div className="nxmp-card-cond">{p.condition||'nuevo'}</div>
                        <div className="nxmp-card-name">{p.titulo}</div>
                        <div className="nxmp-card-price">${(parseFloat(p.price)||0).toFixed(2)}</div>
                        <div className="nxmp-card-stars">{stars(p.rating)}</div>
                        <div className="nxmp-card-foot">
                          <span className={`nxmp-avail ${p.stock>0?'ok':'out'}`}>{p.stock>0?'● Disponible':'● Agotado'}</span>
                          <span className="nxmp-edit">Editar →</span>
                        </div>
                      </div>
                    </div>
                  ))}
